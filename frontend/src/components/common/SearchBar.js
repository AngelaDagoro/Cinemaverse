...
const SearchBar = ({items,label}) => {
    ...
    const handleSearch = (e) => {
        ...
        if(e.key === 'Enter'){
            ...
            if (path === '/movies/admin') {dispatch(clearMovie()); dispatch(getAllMovies({...keywords})); return;}
    ...
    const handleOnChange = (e) => {
        ...
        if (path === '/movies/admin') return dispatch(setKeywords({...keywords, title: e.target.value}));
    ...
    const handleAutoComplete = (event,value,reason) => {
        ...
        if (path === '/movies/admin') return dispatch(setKeywords({...keywords, title: value}));
