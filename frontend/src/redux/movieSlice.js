...
export const addMovie = createAsyncThunk(
    ...
    async(obj, {rejectWithValue}) => {
        try {
            console.log(obj);
            const response = await axios.post(`/api/v1/movies/new`,obj, {
                headers: {
...
export const editMovie = createAsyncThunk(
    ...
    async(obj, {rejectWithValue}) => {
        try {
            console.log(obj);
            const response = await axios.put(`/api/v1/movies/${obj.get('id')}`,obj, {
                headers: {
                    'Content-Type': 'Application/json'
                }
            });
...
export const addActorMovie = createAsyncThunk(
    'movie/addActorMovie',
    async(obj, {rejectWithValue}) => {
        try {
            console.log(obj.get('id'));
            console.log(obj.get('actors'));
            const response =  await axios.post(`/api/v1/movies/addactor/${obj.get('id')}`,obj, {
                headers: {
                    'Content-Type': 'Application/json'
                    }
                });
...
const movieSlice = createSlice({
    ...
    [editMovie.fulfilled] : (state,action) => {
        const movieIndex = state.movies.findIndex(act => act._id === action.payload.movie._id);
    ...
    [addActorMovie.pending] : (state) => {
        state.isLoading = true
    },
    [addActorMovie.fulfilled] : (state,action) => {
        state.movies = [...state.movies, ...action.payload.movies]
        state.moviesCount = action.payload.moviesCount
        state.resPerpage = action.payload.resPerPage
        state.filteredMoviesCount = action.payload.filteredMoviesCount
    ...
    [addActorMovie.rejected] : (state,action) => {
        state.errors = action.payload
        state.isLoading = false
    }   
}
})
export const { setHasMore, setPage, clearMovie, clearMovieEdit } = movieSlice.actions
