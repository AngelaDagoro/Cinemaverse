...
exports.createMovie = catchAsyncErrors(async(req, res, next) => {
    ...
    const { title, type, genre, plot, gross, producers, actors} = req.body
...
exports.addActorMovie = catchAsyncErrors(async (req,res,next) => {

    const movie = await Movie.findById(req.params.id);

    const actors = movie.actors;
    console.log(actors);
    const addactors = {name:req.body.actors};
    console.log(addactors);
    actors.push(addactors);
    await Movie.updateOne({"_id":req.params.id},{"actors":actors},{runValidators:true});
